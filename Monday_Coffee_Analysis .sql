...
SELECT 
	...
	ROUND(
			CAST(SUM(s.total) AS NUMERIC)/
				CAST(COUNT(DISTINCT s.customer_id) AS NUMERIC)
			,2) as avg_sale_pr_cx
...
SELECT 
	...
	ct.avg_sale_pr_cx,
	ROUND(
		CAST(cr.estimated_rent AS NUMERIC)/
								CAST(ct.total_cx AS NUMERIC)
...
SELECT
	...
	last_month_sale,
	ROUND(
		CAST((cr_month_sale-last_month_sale) AS NUMERIC)/CAST(last_month_sale AS NUMERIC) * 100
		, 2
		) as growth_ratio
FROM growth_ratio
WHERE 
